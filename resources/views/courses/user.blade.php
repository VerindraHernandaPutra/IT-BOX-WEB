...
<head>
    ...
    <title>Kursus Saya - ITCOURSE</title>
...
<body>
    ...
    <section class="hero-section">
        <div class="container">
            <h1 class="hero-title">Kursus Saya</h1>
            <p class="hero-subtitle">Kursus yang telah Anda ikuti akan ditampilkan di sini.</p>
            <div class="row">
                @forelse($courses as $course)
                    <div class="col-md-4 mb-4">
                        <div class="card course-card">
                            @if ($course->thumbnail)
                                <img src="{{ asset('storage/' . $course->thumbnail) }}" alt="Thumbnail" class="card-img-top" style="height: 200px; object-fit: cover;">
                            @else
                                <img src="{{ asset('images/default-thumbnail.png') }}" alt="Default Thumbnail" class="card-img-top" style="height: 200px; object-fit: cover;">
                            @endif
                            <div class="card-body">
                                <h5  class="card-title" >{{ $course->course_name }}</h5>
                                <p  class="card-text">{{ $course->description }}</p>
                                <a href="{{ route('courses.materials', $course->id) }}" class="btn btn-secondary">Lihat Materi</a>
                            </div>
                        </div>
                        
                    </div>
                @empty
                    <p class="text-center">Anda belum mendaftar kursus.</p>
                @endforelse
            </div>
        </div>
    </section>
